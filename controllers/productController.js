...
const renderProducts = async (req, res) => {
  try {
    ...
    const spain = await Product.find({ origin: "spain" }).collation({
      locale: "en",
      strength: 2,
    });
    const italy = await Product.find({ origin: "italy" }).collation({
      locale: "en",
      strength: 2,
    });
    const france = await Product.find({ origin: "france" }).collation({
      locale: "en",
      strength: 2,
    });
    let origin;
