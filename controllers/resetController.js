...
const submitReset = async (req, res) => {
  ...
  try {
    if (!user) {
      req.flash(
        "warning_msg",
        "This email is not registered. Please try again or go to register for a new account."
      ),
        res.redirect("/reset");
    }
    ...
    const msg = await {
      ...
      html: `<h2> Click  <a href="https://bubblifywebshop.herokuapp.com/reset/${user.mailToken}"> <b>here</b></a> to create a new password </h2>`,
    };
    sgMail.send(msg);

    req.flash(
      "success_msg",
      "An email has been sent to your adress, please check your inbox."
    );
    res.redirect("/reset");
  } catch (err) {
    console.log(err);
    req.flash(
      "warning_msg",
      "An error has occured. Please try sending the email again."
    );
    res.redirect("/reset");
  }
};
...
const submitFormReset = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send("Something went wrong, please reload page and try again.");
  }
};
